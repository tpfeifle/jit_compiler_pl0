...
TEST(Ast, TestSymbolTable) {
    ...
    for (const auto& var:expectedVariables) {
        symbolName = var.first;
        EXPECT_NE(ast.symbolTable.find(symbolName), ast.symbolTable.end());
        EXPECT_EQ(ast.symbolTable.at(symbolName).first.type, var.second);
    }
}
//---------------------------------------------------------------------------
} // namespace pljit_ast
