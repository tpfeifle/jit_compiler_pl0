...
namespace ast {
//---------------------------------------------------------------------------
unique_ptr<ASTNode> AST::analyzeParseTree(shared_ptr<PTNode> root)
// analyze the ParseTree: This is the starting-point for the analysis
{
    ...
    for (unsigned i = 0; i < (*statementList).children.size(); i++) {
        ...
        if (ASTstatement->statementType == StatementAST::StatementType::Return) {
            hasReturn = true;
        }
...
//---------------------------------------------------------------------------
unique_ptr<ASTNode> AST::analyzeDeclarations(NonTerminalPTNode* node, Symbol::Type type) {
    // TODO: maybe do not use a simple pointer (always use unique/shared ptr)
    NonTerminalPTNode* declaratorList = static_cast<NonTerminalPTNode*>(node->children[1].get());
    for (unsigned i = 0; i < (*declaratorList).children.size(); i++) {
        if ((*declaratorList).children[i]->getType() == PTNode::Type::Identifier) {
            // for declarator
            IdentifierPTNode* identiferToken = static_cast<IdentifierPTNode*>((*declaratorList).children[i].get());
            string identifier = identiferToken->getName();
            if (symbolTable.find(identifier) == symbolTable.end()) {
                bool initialize = type == Symbol::Type::Param;
                Symbol symbol = Symbol(type, (*declaratorList).children[i]->source, initialize);
                symbolTable.emplace(pair<string, Symbol>(identifier, symbol));
            } else {
...
//---------------------------------------------------------------------------
unique_ptr<ASTNode> AST::analyzeConstDeclarations(NonTerminalPTNode* node, Symbol::Type type) {
    ...
    for (unsigned i = 0; i < (*initDeclaratorList).children.size(); i++) {
        ...
        string identifier = identiferToken->getName();
        Symbol symbol = Symbol(type, (*initDeclarator).children[i]->source, true);
        if (symbolTable.find(identifier) == symbolTable.end()) {
            symbolTable.emplace(pair<string, Symbol>(identifier, symbol));
        } else {
            identiferToken->source.printContext(
...
//---------------------------------------------------------------------------
unique_ptr<ASTNode> AST::analyzeStatement(NonTerminalPTNode* node) {
    ...
    if (statement->chosenAlternation == PTNode::Type::AssignmentExpr) {
        ...
        if (symbolTable.at(identifier).type == Symbol::Type::Const) {
            identiferToken->source.printContext("Assigning a value to a constant");
            return nullptr;
            // exit(-1);
        }
        symbolTable.at(identifier).initialized = true;
...
//---------------------------------------------------------------------------
unique_ptr<ASTNode> AST::analyzeAdditiveExpression(NonTerminalPTNode* node) {
    if (node->children.size() == 1) {
        ...
        if (additiveOperator->operatorType == OperatorAlternationPTNode::Plus) {
...
//---------------------------------------------------------------------------
unique_ptr<ASTNode> AST::analyzeMultiplicativeExpression(NonTerminalPTNode* node) {
    ...
    if (primaryExpr->chosenAlternation == PTNode::Type::Identifier) {
        ...
        if (!symbolTable.at(identifier).initialized) {
        ...
        left = make_unique<LiteralAST>(literal->value);
    } else {
    ...
    if (unaryChildIndex == 1) { //this means we had a plus or minus optional in the unary expression
        ...
        if (signOperator->operatorType == OperatorAlternationPTNode::Plus) {
    ...
    if (node->children.size() == 3) {
        ...
        if (muldivOperator->operatorType == OperatorAlternationPTNode::Multiply) {
